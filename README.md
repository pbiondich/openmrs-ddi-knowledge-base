...
- **RxNorm** (NLM) supplies drug-name normalization. All 2,283 drugs resolved to an RxCUI: 94% by exact name match, the rest by stripping dose-form qualifiers or a flagged approximate match. Each drug records how it matched, so lower-confidence matches are auditable.
...
- **RxCUIs are ingredient-level and first-match.** Each drug's RxCUI is the first concept RxNorm returns for its name, normally the ingredient. A handful may point at a more specific concept; `rxnorm_name` makes those easy to spot.
