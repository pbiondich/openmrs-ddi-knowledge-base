...
| `fetch_brand_names.py` | The network step that refreshes `src/brand_names.jsonl` from RxNorm's brand-name concepts per ingredient, applying the exclusion rules (`--refilter` re-applies them offline); what it dropped, and why, is `src/brand_names_excluded.json`. |
...
| Brand names | 1,642 / 2,283 drugs (71.9%) carry at least one RxNorm brand name; 3,863 distinct brands, 6,760 links, after 203 exclusions (`src/brand_names_excluded.json`) |
